...
class AppPublish:
    ...
    def open(self):
        self.parent.close()
        self.ui = util.load_ui_engine(name='publish', target=self.parent)
        self.parent.ui.main_layout.addWidget(self.ui)
        self.parent.setFixedSize(600, 350)
        self.navigator(title="PUBLISH FILE", subtitle="Publishing file")
    ...
    def populate(self):
        ...
        for abc in collect:
            item_mesh = QtWidgets.QTreeWidgetItem(item_asset)
    ...
    def publish(self):
        try:
            ...
            publish_model = published.PublishedModel()
            info = util.get_info_context(ctx, prj, "publish")
            ...
            published_files = publish_model.all()
            publish_existent = list(filter(lambda pub: pub.get("context_id") == ctx.get("_id") and pub.get("step") == ctx.get("step") and pub.get("engine") == ctx.get("engine"), published_files))

            if len(publish_existent) == 0:
                data_publish = {
                    "_id": ctx.get("_id"),
                    "path": path,
                    "path_storage": path.split(prj.get("name"))[1],
                    "name": ctx.get("name"),
                ...
                pub_index = next((index for (index, d) in enumerate(published_files) if d["_id"] == ctx.get("_id")), None)
                published_files[pub_index]["updated"] = util.current_date()
                published_files[pub_index]["version"] += 1
                data_publish = published_files[pub_index]
            ...
            self.parent.publish_scene(path, ctx, prj)
            QtWidgets.QMessageBox.question(self.parent, "Published", "{} published successfuly!".format(os.path.basename(path)), QtWidgets.QMessageBox.Ok)
            self.parent.close()
